Attribute VB_Name = "Rule01_BritishSpelling"
' ============================================================
' Rule01_BritishSpelling.bas
' Proofreading rule: detects US English spellings and suggests
...
Public Function Check_BritishSpelling(doc As Document) As Collection
    ...
    For Each fn In doc.Footnotes
        Err.Clear
        SearchRangeForUSSpellings fn.Range, doc, usWords, ukWords, exceptions, issues
    ...
    For Each en In doc.Endnotes
        Err.Clear
        SearchRangeForUSSpellings en.Range, doc, usWords, ukWords, exceptions, issues
        If Err.Number <> 0 Then Err.Clear
    Next en
    On Error GoTo 0
...
Private Sub SearchRangeForUSSpellings(searchRange As Range, _
    ...
    For i = LBound(usWords) To UBound(usWords)
        ...
        ' Loop through all occurrences of this US term
        Do
            On Error Resume Next
...
' ════════════════════════════════════════════════════════════
Public Sub RunBritishSpelling()
    If ActiveDocument Is Nothing Then
        MsgBox "Please open a document first.", vbExclamation, "British Spelling"
        Exit Sub
    End If
    ...
    Set issues = Check_BritishSpelling(doc)
    ...
    MsgBox "Found " & issues.Count & " issue(s).", _
           vbInformation, "British Spelling"
End Sub
